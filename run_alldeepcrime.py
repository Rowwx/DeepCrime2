...
def run_automate(subject_name, subject_path, subject_path_train, msc): # msc = 'DC' or 'DPP' or 'DM' 18.01 juan
    ...
    #data['mutations'] = ["change_batch_size"] #EU DC:ok (11.01) DPP:ok DM:ok  
    ...
    #data['mutations'] = ["change_weights_initialisation"] #EL DC:ok (11.01) DPP:ok DM:ok
...
###############################################################################################################
    ...
    if props.MS == 'DC_MS':
        ...
        # test ms 15.01.2025
        test_results = os.path.join(data['root'], "mutated_models_"+msc,  data['subject_name'], 'results') # mutated_models_"+msc for separate folders
        
        #print("\nsave_paths[mutated] folder : " +save_paths['mutated']+"\n")
        ...
        if os.path.isdir(test_results):
            ...
            raise Exception()
        # test ms 15.01.2025
        train_accuracy_dir = os.path.join("mutated_models_"+msc, data['subject_name'], "results_train") # mutated_models_"+msc for separate folders
        accuracy_dir = os.path.join("mutated_models_"+msc, data['subject_name'], "results_test") # mutated_models_"+msc for separate folders 
