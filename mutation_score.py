...
from datetime import datetime # for datetime csv 18.01 juan
        ...
        if '.csv' in filename:
            ...
            if test_score != -1:
                score_dict[filename.replace(stats_dir + os.path.sep, '')] = {'test_score': test_score,
                                                                             'ins_score': ins_score} # computation time 18.01 juan
...
def get_upper_bound(file_short_name, lower_lr):
    if 'delete_training_data' in file_short_name: # changed 18.01
        return 99
...
def is_binary_search_operator(filename):
    operator_list = ['change_label', 'delete_training_data', 'unbalance_train_data', 'add_noise', 'output_classes_overlap', 'change_epochs', 'change_learning_rate', 'change_patience'] # delete_td and unbalance_td changed 18.01 
...
def calculate_ms(train_accuracy_dir, accuracy_dir, comp_time, msc): # msc = 'DC' or 'DPP' or 'DM' 18.01 juan
    """
    Calculates mutation score and saves it to a CSV file.

    Args:
        train_accuracy_dir: Path to the training accuracy directory.
        accuracy_dir: Path to the accuracy directory.
        comp_time: Computation time for the mutation operator.
        msc: String representing the mutation selection criteria (e.g., "DC", "DM", "DM++").
    Returns:
        None
    """
    ...
    results_dir = "/home/jovyan/BT_test/deepcrime2/results/" 
    os.makedirs(results_dir, exist_ok=True)  # Create the directory if it doesn't exist

    ms_csv_file = os.path.join(accuracy_dir, subject_name + "_" + msc + "ms.csv") 
    ...
    ct_csv_file = os.path.join(results_dir, subject_name + "_" + msc + "ct.csv") 

    with open(ct_csv_file, 'a') as f1: # save computation time to results, append consequent files with datetime
        writer = csv.writer(f1, delimiter=',', lineterminator='\n', )
        # Get current timestamp
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        # Write timestamp and filename as the first row
        writer.writerow(['operator_name', 'operator_ms', 'computation_time']) 
        for key, value in score_dict.items():
            writer.writerow([key, value['test_score'], comp_time]) 
        writer.writerow(['total MS:', mut_score, timestamp]) 
        print(f"\n File added! at {timestamp} for file: {ct_csv_file}\n")
...
def calculate_dc_ms(train_accuracy_dir, accuracy_dir):
    ...
    ms_csv_file = os.path.join(accuracy_dir, subject_name + "_DCms.csv") # changed to save as DC ms. 15.01
...
# function for calculating DM++ mutation score 15.01.2025 juan
def calculate_dpp_ms(train_accuracy_dir, accuracy_dir):
    dc_props = read_properties()
    subject_name = dc_props['subject_name']
    prefix = subject_name
    ...
    ms_csv_file = os.path.join(accuracy_dir, subject_name + "_DPPms.csv") # changed to save as DPP ms. 15.01
...
# function for calculating DM mutation score 15.01.2025 juan
def calculate_dm_ms(train_accuracy_dir, accuracy_dir):
    dc_props = read_properties()
    subject_name = dc_props['subject_name']
    prefix = subject_name
    ...
    ms_csv_file = os.path.join(accuracy_dir, subject_name + "_DMms.csv") # save as DM ms. 15.01
