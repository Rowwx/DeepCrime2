...
def modify_original_modelA(model_path):
    with open(model_path, "r") as source:
        tree = ast.parse(source.read())
    ...
    # Unparse the AST tree and write the resulting code back into the same _orig.py file
    # This only modifies the existing file, it does not generate a new one
    #unparse_tree(tree, save_path_prepared)
