...
from stats import is_accuracy_threshold 
from run_deepcrime_properties import read_properties

import numpy as np # for DeepMutation++ 15.01 juan
...
def loadoriginaldata(dataset_name="mnist"):
    ...
    if dataset_name == "mnist":
        ...
        x_test = x_test.astype('float32') / 255  # Reshape and normalize
    else:
...
def execute_based_on_search(udp, search_type, mutation, mutant, mutation_params, ind = None, mutation_ind = ''):
    ...
    try:
        if udp and (search_type is None): # 11.01 changed from or -> and 
            ...
            is_sts, p_value, effect_size = is_accuracy_threshold(original_accuracy_list, mutation_accuracy_list)
...
def execute_binary_search(mutant, mutation, my_params):
    ...
    is_sts, p_value, effect_size = is_accuracy_threshold(original_accuracy_list, upper_accuracy_list)
...
def search_for_bs_conf(mutant, mutation, my_params, lower_bound, upper_bound, original_accuracy_list):
    ...
    is_sts, p_value, effect_size = is_accuracy_threshold(original_accuracy_list, middle_accuracy_list)
