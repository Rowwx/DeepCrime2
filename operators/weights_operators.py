...
def operator_change_weights_initialisation(model):
    ...
    if tmp['layers'][current_index]['config'].get('kernel_initializer'):
        ...
        # Ensure that if the initializer is 'constant', a 'value' field is included 27.11 by juan
        if isinstance(new_init, str) and new_init.lower() == 'constant':
            # If it's a constant initializer, we need to specify the value (e.g., 0.0)
            new_init = {
                'class_name': 'Constant',
                'config': {'value': 0.0}  # You can modify this value as needed
            }
        # Ensure that if the initializer is 'constant', a 'value' field is included 27.11 by juan
